...
class Signal extends PureRenderComponent{
  constructor(props){
    super(props);
    this.statexbar_style = default_bar_style;
  ...
  render(){
    ...
    var is_self = from_lifeline.get("actor") == to_lifeline.get("actor");
    if(is_self){
    ...
    var bar_style;
    if(signal_data.get("is_replay_on")){
    ...
    return(
      <g ref="signal_g" transform={"translate(0," + signal_data.get("y") +")"}>
        <text x={text_x} y={0}>{to_lifeline.get("method_name")}</text>
